...
std::string saveDirectory = "/home/ralkahal/nnew-tudat-tests/covAn/twowaydoppler_apriori/accumsResults_empirical_CS_drag/";
...
void computeCovarianceMatrix(int startTime, int nIndex,int numberOfLocalParameters, int numberOfGlobalParameters, std::vector<Eigen::MatrixXd> normalizedDesignMatrices,std::vector<Eigen::MatrixXd> unnormalizedDesignMatrices,std::vector<Eigen::VectorXd> normalizationFactors, std::vector<Eigen::VectorXd> weightDiagonals,std::vector<Eigen::MatrixXd> P0_matrices) {
	...
	std::cout<<"covariance analysis for" << nIndex <<" arcs done, starting summing them up..."<<std::endl;
        ...
        int nLocal=numberOfLocalParameters-6;
...
*/
	...
	std::vector<Eigen::MatrixXd> inverseNormalizedCovarianceMatrices;
		...
		std::vector<double> filteredInitialTimes;
...
*/
		...
		int numberOfLocalParameters = 7+lengthOfTimeListEmp*6;
                numberOfGlobalParameters = numberOfParameters - numberOfLocalParameters;
		...
		// adding 6 empirical accelerations per revolution = 6*listEmpsTimes
		...
		if ((i + 1) % batchSize ==0 || i == numberOfIntegrationArcs -1){
		    computeCovarianceMatrix(startTime,i+1,numberOfLocalParameters,numberOfGlobalParameters,normalizedDesignMatrices,unnormalizedDesignMatrices,normalizationFactors,weightDiagonals,P0_matrices);
		}
        }
...
int main() {
	...
	int batchSize = 73;//365;
        std::vector<int> arcLengths= {5};
        std::vector<int> number_of_arcs= {73};//{3650};//72,137,210};//{210,283,356};
        ...
        std::vector<double> initialTimes = {0.0*86400.0};//{0.0*86400.0};
        ...
        std::vector<int> totalDuration = {365};//,685,1050}; //{1050,1415,1780};
        std::vector<double> finalTimes= {86400.0*(365.0)};//86400.0*18250.0};//,//,86400.0*685.0,86400.0*1050.0}; //{86400.0*1050.0,86400.0*1415.0,86400.0*1780.0};
