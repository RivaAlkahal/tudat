...
void arcLengthRuns( double hoursperday, double initialTime, double finalTime, int arcLength, int iterationNumber, double perturbPos, double perturbVel, int number_of_arcs, int itotalDuration, int startTime, int intperturbPos, int intperturbVel, int ihoursperday, bool performEst )
{
    ...
    std::string saveDirectory = "/home/ralkahal/nnew-tudat-tests/covAn/twowaydoppler_apriori/";    //std::string fileTag = "arcLengths_160darc_startat320_0per_10hobs_6itr";
    std::string fileTag = "234polyperiodicand80Static_" +  std::to_string(arcLength) + std::to_string(itotalDuration) +  "darc_startat" + std::to_string(startTime)
 ...
 cosineShAmplitudesCosineTime.push_back(
    ...
    int numberOfIntegrationArcs = integrationArcStartTimes.size( );
        ...
        parameterNames.push_back( std::make_shared< SphericalHarmonicEstimatableParameterSettings >(
                                 2, 0, 80, 80, "Mars", spherical_harmonics_cosine_coefficient_block ) );
        parameterNames.push_back( std::make_shared< SphericalHarmonicEstimatableParameterSettings >(
                                      2, 1, 80, 80, "Mars", spherical_harmonics_sine_coefficient_block ) );
...
// Create the noise functions that return Eigen::VectorXd
	...
	matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+23, DIAGONALS+cnmErrors.size()+snmErrors.size()+23) = 1.0/(0.010E-09*0.010E-09);
        ...
        matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+24, DIAGONALS+cnmErrors.size()+snmErrors.size()+24) = 1.0/(0.1E-19*0.1E-19);
        matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+25, DIAGONALS+cnmErrors.size()+snmErrors.size()+25) = 1.0/(0.1E-19*0.1E-19);
        matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+26, DIAGONALS+cnmErrors.size()+snmErrors.size()+26) = 1.0/(0.1E-19*0.1E-19);
        matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+27, DIAGONALS+cnmErrors.size()+snmErrors.size()+27) = 1.0/(0.1E-19*0.1E-19);
	matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+28, DIAGONALS+cnmErrors.size()+snmErrors.size()+28) = 1.0/(0.1E-19*0.1E-19);
...
int main() {
        ...
        std::vector<int> totalDuration = {60};
        std::vector<double> finalTimes= {86400.0*60.0};
