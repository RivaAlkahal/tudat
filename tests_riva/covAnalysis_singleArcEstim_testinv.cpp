//
// Created by ralkahal on 10-3-25.
//
//
...
void arcLengthRuns( double hoursperday, double initialTime, double finalTime, int arcLength, int iterationNumber, double perturbPos, double perturbVel, int number_of_arcs, int itotalDuration, int startTime, int intperturbPos, int intperturbVel, int ihoursperday, bool performEst, bool filterArcTimes )
{
    ...
    std::string saveDirectory = "/home/ralkahal/nnew-tudat-tests/covAn/twowaydoppler_apriori/manualSetupTest_inverse/";    //std::string fileTag = "arcLengths_160darc_startat320_0per_10hobs_6itr";
    ...
    std::string fileTag = "summationsDEBUGDragperarc_" +  std::to_string(arcLength) + std::to_string(itotalDuration) +  "darc_startat" + std::to_string(startTime)
                                  + "_" + std::to_string(finalTime) + "_" + std::to_string(intperturbPos) + "perpos_" + std::to_string(intperturbVel) + "_pervel_" + std::to_string(ihoursperday) + "hobs_" + std::to_string(iterationNumber) + "itr_spiceprop" ;
...
*/
	...
	if (filterArcTimes){
        ...
        std::vector<std::vector<double>> observationTimesPerArc(numberOfIntegrationArcs);
        ...
        std::vector< Eigen::MatrixXd > covarianceMatrices(numberOfIntegrationArcs);
        //std::vector<Eigen::MatrixXd> inverseUnnormalizedCovarianceMatrices;
        std::vector<Eigen::VectorXd> weightDiagonals(numberOfIntegrationArcs);
        std::vector<Eigen::MatrixXd> unnormalizedDesignMatrices(numberOfIntegrationArcs);
	std::vector<Eigen::MatrixXd> normalizedDesignMatrices(numberOfIntegrationArcs);
        int numberOfGlobalParameters;
	std::vector<Eigen::MatrixXd> inverseNormalizedCovarianceMatrices(numberOfIntegrationArcs);
	...
	for( unsigned int i = 0; i < numberOfIntegrationArcs; i++ )
		...
		std::ofstream feo(saveDirectory + "observationTimes_arc_" + std::to_string(i) +  fileTag + ".txt");
			...
			feo <<  std::setprecision(32) <<time <<"\n";
                ...
                feo.close();
		observationTimesPerArc[i] = filteredObservationTimes;
		...
		Eigen::MatrixXd unnormalizedDesignMatrix = covarianceOutput->getUnnormalizedDesignMatrix( );
                ...
                //fe.close();
		unnormalizedDesignMatrices[i] = unnormalizedDesignMatrix;
		...
		Eigen::MatrixXd normalizedDesignMatrix = covarianceOutput->getNormalizedDesignMatrix( );
                ...
                // Close the file
                //fe2.close();
                normalizedDesignMatrices[i] = normalizedDesignMatrix;

                Eigen::VectorXd weightMatrixDiagonal = covarianceOutput->weightsMatrixDiagonal_;
                weightDiagonals[i] = weightMatrixDiagonal;
                ...
                inverseNormalizedCovarianceMatrices[i] = inverseNormalizedCovarianceMatrix;
...
// fill in the global matrix
        ...
        Eigen::MatrixXd NormalizedCovarianceMatrixSummed = P_global.inverse() ;
...
int main() {
        ...
        std::vector<int> number_of_arcs= {2};
        ...
        std::vector<int> totalDuration = {10};
        std::vector<double> finalTimes= {86400.0*10.0};
