...
*/
	...
	if (count<2){
	    ...
	    std::cout<<"filteredArcStartTimes:"<<currentStartTime<<std::endl;
	    std::cout<<"filteredArcEndTimes:"<<currentEndTime<<std::endl;
	}
	...
	if (filterArcTimes){
                ...
                arcPropagationSettingsList.push_back(
                        std::make_shared< TranslationalStatePropagatorSettings< double > >
                                ( centralBodies, accelerationModelMap, bodiesToIntegrate,
                                  systemInitialStates.at(i), arcEndTimesToEstimate.at( i ), cowell, dependentVariablesToSave, TUDAT_NAN ) );
        }
    	...
    	std::vector< std::shared_ptr< EstimatableParameterSettings > > parameterNames =
    	    getInitialMultiArcParameterSettings< double, double  >( multiArcPropagatorSettings, bodies, arcTimesToEstimate );
	...
	std::cout<<" cosine block indices per period size"<<cosineBlockIndicesPerPeriod[0].size()<<std::endl;
       ...
       // cosineBlockIndicesPerPower[ 1 ].push_back( std::make_pair( 5, 0 ) );
        ...
        parameterNames.push_back(std::make_shared< ArcWiseDragCoefficientEstimatableParameterSettings >(spacecraftName,integrationArcStartTimes));// initial_times_list_drag ));
...
// Create the noise functions that return Eigen::VectorXd
	...
	matrix(DIAGONALS+cnmErrors.size()+snmErrors.size()+28, DIAGONALS+cnmErrors.size()+snmErrors.size()+28) = 0.0;///(0.1E-19*0.1E-19);
        ...
        for (int i = DIAGONALS+cnmErrors.size()+snmErrors.size()+29; i < remainingParametersForDrag; ++i) {
                matrix(i,i) = aprioriuncertainty;
        }
                // Fill the matrix with the values of the diagonal
...
int main() {
        ...
        std::vector<int> number_of_arcs= {24};
        ...
        std::vector<int> totalDuration = {120};
        std::vector<double> finalTimes= {86400.0*120.0};
        bool performEst = false;
	bool filterArcTimes = true;
